...
function FisikEdu() {
  ...
  const [categoryTitle, setCategoryTitle] = useState('Kesehatan Fisik'); // Default title
  const [categoryIcon, setCategoryIcon] = useState(KesehatanFisik); // Default icon
  const { category } = useParams(); // Get category from URL (e.g., "KesehatanFisik")

  useEffect(() => {
    // Map URL category to API-friendly format
    const formattedCategory = category?.replace(/\s+/g, '-').toLowerCase() || 'kesehatan-fisik';
      ...
      .then(data => {
        if (data.status) {
          ...
          if (matchedCategory) {
            ...
            setCategoryIcon(matchedCategory.image || KesehatanFisik); // Set category icon
        ...
        <section className="w-full px-4 md:px-20 py-5 bg-white">
          ...
          <div className="text-center mb-12">
            <img
              ...
              className="mx-auto w-40 h-40 mb-2 object-contain"
              onError={(e) => (e.target.src = KesehatanFisik)} // Fallback icon
            />
                  ...
                  >
                    ...
                    <div className="w-full md:w-2/5">
                      <img
                        ...
                        onError={(e) => (e.target.src = KesehatanFisik)} // Fallback image
                      />
                    </div>
                  </div>
                ))
