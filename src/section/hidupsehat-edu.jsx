...
function HidupSehat() {
  ...
  const [categoryTitle, setCategoryTitle] = useState('Pola Hidup Sehat'); // Default title
  const [categoryIcon, setCategoryIcon] = useState(Sleepy); // Default icon
  const { category } = useParams(); // Get category from URL (e.g., "PolaHidupSehat")

  useEffect(() => {
    // Map URL category to API-friendly format
    const formattedCategory = category?.replace(/\s+/g, '-').toLowerCase() || 'pola-hidup-sehat';
      ...
      .then(data => {
        if (data.status) {
          ...
          if (matchedCategory) {
            ...
            setCategoryIcon(matchedCategory.image || Sleepy); // Set category icon
  ...
  console.log("KebersihanDiri component rendered");
                  ...
                  >
                    ...
                    <div className="w-full md:w-2/5">
                      <img
                        ...
                        onError={(e) => (e.target.src = Sleepy)} // Fallback image
                      />
                    </div>
                  </div>
                ))
